...
do
  ...
  then
    jsonvalue="$(echo "{\"Name\":\"${service_to_register}\", \"Address\":\"${host_ip}\", \"Tags\": [\"kong\"], \"Port\":$nginx_port, \"Check\": "{\"Name\": \"Check Script\", \"Script\": \"echo true\", \"Interval\": \"10s\"}"}")"

    echo "Registering service $service $nginx_port $jsonvalue"
    curl -X PUT --header "Content-Type: application/json" -d "$jsonvalue" "http://$consulService:8500/v1/agent/service/register"
  fi
done
